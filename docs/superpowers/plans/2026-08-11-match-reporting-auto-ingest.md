...
**Goal:** Captains file match reports on the site; a 2am GitHub Actions job ingests them into `raw_stats` with the correct team, season and phase — no local runs, no manual match-id typing.
...
- Gates every task: `npm run build`, `npm run lint` exit 0, `npm test`, `npx supabase test db` (if pgTAP flakes on `0010`, do `npx supabase db reset` + `npx tsx scripts/load-stats.ts` then retest), plus `python scripts/test_riot_stats_ingest.py` for Python-touching tasks. One `npm run e2e` in Tasks 4 and 8 (retry once on the known draft warm-up flake).
- Commits end with `Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>`.
- Production rollout (Task 8) targets project ref `tyywoneobreracfnujdk` ONLY. `jmhgextkwsaodtnjtvvp` ("Draft League") must never be touched.
...
src/components/admin/LeagueSettingsEditor.tsx          (T6)
src/components/admin/LeagueTeamsEditor.tsx             (T6)
src/components/admin/RosterEditor.tsx                  (T6)
src/app/admin/page.tsx                                 (T6 — link/mount the three editors)
...
**Interfaces:**
- Produces:
  - `types.ts`: `LeagueSettings`, `LeagueTeam`, `RiotAccount`, `RosterMembership`, `MatchReport`, `MatchReportGame` (mirror T1/T2 columns exactly), plus `ReportStatus`/`GameStatus` unions.
...
- Produces: `queries.ts` exports `fetchLeagueTeams(): Promise<LeagueTeam[]>`, `fetchLeagueSettings(): Promise<LeagueSettings | null>`, `fetchReports(limit?: number): Promise<(MatchReport & { games: MatchReportGame[] })[]>`, `fetchExistingMatchIds(ids: string[]): Promise<string[]>` (checks `match_report_games` AND `raw_stats`), `submitReport(input: SubmitReportInput): Promise<{ reportId: string }>` where `SubmitReportInput = { season: string; phase: string; teamAId: string; teamBId: string; scoreA: number; scoreB: number; draftUrl: string | null; games: { gameNumber: number; matchId: string; blueTeamId: string | null }[] }` (inserts the report then its games; on failure of the games insert, deletes the report row so no orphan queue entry survives).
- Form behaviour: page is a server component that gates on sign-in (`createServerSupabase().auth.getUser()`; signed-out shows a branded "Sign in to report" card with a `/login` link) and renders `ReportForm` with teams + settings fetched server-side. `ReportForm` (client): paste textarea → "Parse" button → fills team selects, score inputs, season/phase (defaults from settings), draft url, and a game row list (game number, match id, blue-side select with options Auto-detect / Team A / Team B). Rows are addable/removable manually. Submit validates: both teams set and different, ≥1 game, every match id matching `^NA1_\d+$`, no duplicate ids within the form, and none already known (`fetchExistingMatchIds`) — each failure shown inline. Success → redirect to `/matches`.
...
- Modify: `src/components/SiteNavigation.tsx` (add `Matches` → `/matches`)
...
### Task 6: Admin editors (settings, league teams, rosters)

**Files:**
- Create: `src/components/admin/LeagueSettingsEditor.tsx`, `src/components/admin/LeagueTeamsEditor.tsx`, `src/components/admin/RosterEditor.tsx`
- Modify: `src/app/admin/page.tsx` (mount the three under the existing admin page, each in a `card-brand` section with a `label-dash` heading)

**Interfaces:**
- Consumes: types (T3), browser Supabase client; admin gating already exists on `/admin` (server-side redirect for non-admins).
- Produces: `LeagueSettingsEditor` — two inputs (current season, current phase) + Save (updates the single row). `LeagueTeamsEditor` — table of `league_teams` with editable name/abbreviation/active, add row, delete row (guarded: refuse delete when the team is referenced by a report, surfacing the DB error message). `RosterEditor` — season selector (defaults to `league_settings.current_season`), team selector, list of that team's memberships showing `Name#TAG`, an add box accepting `Name#TAG` (splits on the last `#`; creates the `riot_accounts` row when absent via upsert on the lower-cased pair, then the membership), and a remove button per row; a paste-multiple textarea accepting one `Name#TAG` per line for bulk add.

- [ ] Steps: settings editor → teams editor → roster editor → browser check as admin (add a roster entry, confirm rows in psql; confirm a non-admin is redirected) → gates → commit `feat: admin editors for league settings, teams and rosters`.
...
### Task 8: Verification + production rollout
...
- [ ] **Step 2: Screenshot sweep** — `/matches` (empty state + populated with each status), `/matches/report` (paste → parsed form), admin editors, and the nav entry; desktop 1600w plus one 390w mobile shot of `/matches`. Read every screenshot; fix styling-only issues found.
