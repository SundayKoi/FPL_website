-- REVIEWED OVERRIDE of supabase/migrations/20261018000001_card_art_champion_preferences.sql
--
-- The original cannot be applied on any PostgreSQL: card_art_champion_key()
-- and card_art_champion_display() are `language sql` functions whose bodies
-- are a bare `case ... end` expression, which is not a SQL statement
-- (ERROR: syntax error at or near "case"), so the whole migration rolls back.
-- Migrations are immutable (scripts/check-migrations.mjs), so the original
-- stays as it is and scripts/supabase-migrations.mjs stages this file under
-- the same version and name instead, for `push` and for fresh databases.
--
-- The ONLY difference from the original: each of those two function bodies
-- starts with `select`, i.e. `select case ... end`. Everything after this
-- header is otherwise byte-for-byte the original.
--
-- The wrapper pins the original's git blob id and this file's blob id; if
-- either file changes, staging fails until the override is reviewed again.
-- ---------------------------------------------------------------------------
