...
values ('Test Premier Retired 0050', 'WC', false);

insert into public.teams (draft_id, name, abbreviation, nomination_position, budget_start, points_remaining)
values ((select d from t), 'Test Premier Retired 0050', 'WLD', 90, 100, 100);

update public.league_settings set featured_draft_id = (select d from t) where id = 1;

select is((select active from public.league_teams where name = 'Test Premier Retired 0050'), false,
...
select is((select active from public.league_teams where name = 'Test Premier Retired 0050'), true,
          'a redrafted retired name is brought back into service');

select is((select count(*) from public.league_teams where lower(name) = 'test premier retired 0050'), 1::bigint,
