...
select has_table('public', 'box_score_daily_candidates', 'Box Score candidates exist');
select has_table('public', 'box_score_daily_puzzles', 'Box Score puzzles exist');
select has_table('public', 'box_score_daily_progress', 'Box Score progress exists');
...
select ok(not has_function_privilege('anon', 'public.ensure_box_score_daily_puzzle(date,text,text,jsonb)', 'execute'), 'anon cannot create Box Score puzzles');
select ok(not has_function_privilege('authenticated', 'public.ensure_box_score_daily_puzzle(date,text,text,jsonb)', 'execute'), 'authenticated cannot create Box Score puzzles');
select ok(has_function_privilege('service_role', 'public.ensure_box_score_daily_puzzle(date,text,text,jsonb)', 'execute'), 'service role can create Box Score puzzles');
select ok(not has_function_privilege('anon', 'public.record_box_score_guess(date,text,uuid,text,text)', 'execute'), 'anon cannot record Box Score guesses');
select ok(not has_function_privilege('authenticated', 'public.record_box_score_guess(date,text,uuid,text,text)', 'execute'), 'authenticated cannot record Box Score guesses');
select ok(has_function_privilege('service_role', 'public.record_box_score_guess(date,text,uuid,text,text)', 'execute'), 'service role can record Box Score guesses');
select ok(not has_function_privilege('anon', 'public.reset_box_score_daily_puzzle(date,text)', 'execute'), 'anon cannot reset Box Score puzzles');
select ok(not has_function_privilege('authenticated', 'public.reset_box_score_daily_puzzle(date,text)', 'execute'), 'authenticated cannot reset Box Score puzzles');
select ok(has_function_privilege('service_role', 'public.reset_box_score_daily_puzzle(date,text)', 'execute'), 'service role can reset Box Score puzzles');
select ok(has_function_privilege('service_role', 'public.claim_daily_game_reward(date,uuid,text,text,bigint)', 'execute'), 'service role can claim the shared reward for Box Score');

insert into public.profiles (id, display_name)
values
  ('00000000-0000-0000-0000-000000008001', 'Box Score Player One'),
  ('00000000-0000-0000-0000-000000008002', 'Box Score Player Two');
insert into public.betting_profiles (discord_id, profile_id, username, balance)
values
  ('box-score-0801', '00000000-0000-0000-0000-000000008001', 'Box One', 1000),
  ('box-score-0802', '00000000-0000-0000-0000-000000008002', 'Box Two', 1000);
...
values
  ('BS_MATCH_ONE', '2099-02-07 12:00:00', 35, 'Blue', 'Box Score FC',
  ...
  ('BS_MATCH_TWO', '2099-02-06 12:00:00', 38, 'Red', 'Box Score United',
...
select ok((select puzzle.target_stats ?& array['role', 'champion', 'kills', 'damageSharePct', 'csAt10', 'team', 'date', 'objectives'] from public.box_score_daily_puzzles puzzle where puzzle.puzzle_date = '2099-02-08' and puzzle.league = 'premier'), 'target stores staged box-score fields, not internal raw columns');
...
select * from public.record_box_score_guess(
  '2099-02-08', 'premier', '00000000-0000-0000-0000-000000008002', 'box-score-0802', :'premier_wrong_slug'
)
\gset other_user_
select is(:'other_user_status'::text, 'playing', 'a second user gets independent Box Score progress');
...
select * from public.record_box_score_guess(
  '2099-02-08', 'premier', '00000000-0000-0000-0000-000000008001', 'box-score-0801', :'premier_wrong_slug'
)
\gset wrong_
...
select * from public.record_box_score_guess(
  '2099-02-08', 'premier', '00000000-0000-0000-0000-000000008001', 'box-score-0801', :'premier_before_answer_slug'
)
\gset correct_
...
select is(:'correct_reward_amount'::bigint, 200::bigint, 'Box Score claims the normal shared reward');
select is((select count(*)::int from public.daily_game_rewards where profile_id = '00000000-0000-0000-0000-000000008001' and puzzle_date = '2099-02-08'), 1, 'Box Score creates one shared reward claim');

select * from public.claim_daily_game_reward(
  '2099-02-08', '00000000-0000-0000-0000-000000008001', 'box-score-0801', 'fpldle', 8001
)
\gset cross_game_
select is(:'cross_game_already_claimed'::boolean, true, 'another daily game sees the existing Box Score reward');
select is((select count(*)::int from public.betting_ledger where discord_id = 'box-score-0801' and reason = 'daily_game_reward'), 1, 'cross-game completion does not pay twice');

select throws_ok($$
  select * from public.record_box_score_guess(
    '2099-02-08', 'premier', '00000000-0000-0000-0000-000000008001', 'box-score-0801', 'bs-player-one-na1'
  )
...
select throws_ok($$
  ...
  values ('2099-02-08', 'academy', '00000000-0000-0000-0000-000000008002', 'box-score-0802', array['a', 'b', 'c', 'd', 'e', 'f'])
...
\gset premier_after_
select * from public.record_box_score_guess(
  '2099-02-08', 'premier', '00000000-0000-0000-0000-000000008001', 'box-score-0801', :'premier_after_answer_slug'
)
\gset replay_
select is(:'replay_already_rewarded'::boolean, true, 'reset replay keeps shared reward idempotent');
select is((select count(*)::int from public.betting_ledger where discord_id = 'box-score-0801' and reason = 'daily_game_reward'), 1, 'reset replay does not add a second reward ledger row');
