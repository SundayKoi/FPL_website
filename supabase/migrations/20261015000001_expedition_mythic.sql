-- The Mythic route: above the Legendary route, and the Voidborn.
--
-- Four days, five forks, every one warned, and the only route where the
-- pushes carry (momentum — src/lib/expeditions/routes.ts). Three
-- fragments, a Voidtouched card in the squad and a Legend mark on the
-- shelf open it; a Voidtouched survivor comes home Voidborn, the second
-- stage and the one mutation that replaces another. The app decides
-- everything about the road; the database learns:
--
--   1. 'mythic' is a tier (expedition_runs's check) and 'voidborn' a
--      mutation (card_inventory's check).
--   2. The Mythic route is five miles (expedition_trail_miles).
--   3. launch_expedition knows the tier — in every list a Legendary run is
--      in — and checks the two gates under lock. The 12-argument body is
--      redeclared exactly as 20260929000001 left it apart from that; the
--      13-argument convoy wrapper (20260917000001) delegates to it.
--   4. resolve_expedition knows the tier: a card can die or be lost past
--      the rift, Voidtouched lands on a Legendary or Mythic run, Voidborn
--      only on a Mythic run and only on a card that is Voidtouched (the
--      stamp's where lets it replace), and the payout ceiling follows the
--      Mythic jackpot through maxExpeditionPayout(): 19050.
--      Redeclared in full, exactly as 20261012000001 left it apart from
--      those lines.
...
alter table public.expedition_runs
  ...
  check (tier in ('scout', 'gilded', 'raid', 'legend', 'rescue', 'exorcism', 'legendary', 'mythic', 'lost'));
...
alter table public.card_inventory
  ...
  check (mutation is null or mutation in ('irradiated', 'hardened', 'haunted', 'cursed', 'voidtouched', 'voidborn'));
...
begin
  ...
  if v_dollars not between 0 and 19050 then raise exception 'payout out of range'; end if;
  ...
  for v_fate in select * from jsonb_array_elements(p_outcome -> 'fates') loop
    ...
    if v_mut is not null and v_mut not in ('irradiated', 'hardened', 'haunted', 'cursed', 'voidtouched', 'voidborn') then
      raise exception 'unknown mutation';
    end if;
    if v_kind = 'dead' and v_run.tier not in ('legendary', 'mythic') then raise exception 'fate beyond route'; end if;
    if v_kind = 'lost' and v_run.tier not in ('legend', 'rescue', 'legendary', 'mythic') then raise exception 'fate beyond route'; end if;
    ...
    if v_mut = 'voidtouched' and v_run.tier not in ('legendary', 'mythic') then raise exception 'mutation beyond route'; end if;
    -- Voidborn: only past the rift, and only on a card that is Voidtouched.
    if v_mut = 'voidborn' then
      if v_run.tier <> 'mythic' then raise exception 'mutation beyond route'; end if;
      if not exists (select 1 from card_inventory ci where ci.id = v_id and ci.mutation = 'voidtouched') then
        raise exception 'mutation beyond card';
      end if;
    end if;
  end loop;
  ...
  for v_fate in select * from jsonb_array_elements(p_outcome -> 'fates') loop
    ...
    if v_mut is not null and v_kind <> 'dead' then
      -- One per copy, permanent — except Voidborn, the second stage, which
      -- replaces Voidtouched and nothing else.
      update card_inventory
        set card = jsonb_set(card, '{mutation}', jsonb_build_object('key', v_mut, 'date', to_char(v_stamp, 'YYYY-MM-DD'), 'run', p_run))
        where id = v_id
          and (card -> 'mutation' is null or (v_mut = 'voidborn' and card -> 'mutation' ->> 'key' = 'voidtouched'));
    end if;
