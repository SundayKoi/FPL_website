-- God Packs, applied in order.
--
-- WHY THIS FILE EXISTS. The God Pack work shipped to production with its
-- migration numbered 20260915000001, which sorts BEFORE migrations this
-- database had already applied (through 20260929000001, the Dribb card).
-- Supabase skips a migration older than its recorded high-water mark, so
-- begin_card_pack_opening and its three companions were never created —
-- while the code calling them was live. Every pack open failed on the
-- first call, before any money moved.
--
-- This is that migration renumbered to apply, with one correction: the
-- provenance recorder below keeps what the later migrations added to it.
-- Everything else is byte-for-byte the original.
--
-- Idempotent throughout (create table if not exists, add column if not
-- exists, create or replace function), so it is safe to run whether or not
-- any part of the original ever landed.
...
-- Replace the recorder so the durable opening identity is copied into every
-- mint and transfer-history row, WITHOUT losing what the recorder already
-- carries: the season, the flat `print` the pull rates are read from
-- (20260923000001), the Dribb flag (20260929000001), and the died/dusted
-- distinction the expedition graveyard depends on (20260914000001).
--
-- This is the one place this file differs from the version authored against
-- the older baseline: that version dropped all four. Existing copies keep a
-- null opening_id.
