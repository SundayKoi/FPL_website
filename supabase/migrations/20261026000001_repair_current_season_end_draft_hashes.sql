-- Recompute the current mutable Premier S5 Season's End draft after the
-- application canonical serializer was corrected. The earlier repair ran,
-- but the draft was rebuilt once more by the old application afterward.
--
-- Release UUIDs differ by environment, so select the latest matching draft.
-- Locked admin-test and public revisions are never selected or modified.
do $$
declare
