-- Expedition weather: one league-wide condition a week.
--
-- All of it lives in the app (src/lib/expeditions/weather.ts): the week's
-- weather is drawn from the Eastern Monday's date — Fog (every fork is
-- dark), Drought (the scouting coin flip pays half, caches twice as
-- common), Harvest (merchants pay double, tolls waived), Clear (nothing,
-- on purpose) — and the Watch in a playoff week (rivals on every road,
-- tolls doubled, ghosts in daylight). A run keeps the weather it launched
-- under, derived from its launch week and never stored.
--
-- The database learns two things.
--
--   1. THE RULEBOOK VERSION moves to 5. A run already in the field
--      launched into no weather, because its journal is half written and
--      a fork that turned dark under it would rewrite what the page
--      showed (WEATHER_RULES in weather.ts).
--
--   2. THE PAYOUT CEILING follows the Harvest. resolve_expedition refuses
--      a payout over the most a run can pay, and the merchant's flat is
--      the one thing added after the multipliers — so a Harvest merchant
--      at 150 instead of 75 raises the most a run can pay by 75, from
--      16275 to 16350. maxExpeditionPayout() in config.ts carries the
--      same number (HARVEST_MERCHANT), and 0117 holds the two together.
--      resolve_expedition is re-declared in full, exactly as
--      20261007000001 left it apart from that one number.

alter table public.expedition_runs alter column rules set default 5;
...
begin
  ...
  if v_dollars not between 0 and 16350 then raise exception 'payout out of range'; end if;
