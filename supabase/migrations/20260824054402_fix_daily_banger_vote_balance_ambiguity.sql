-- Qualify betting_profiles.balance so it cannot conflict with the output
-- parameter named balance, and keep this privileged reward RPC service-only.
...
revoke execute on function public.vote_daily_banger(text, uuid, text, text) from public, anon, authenticated;
