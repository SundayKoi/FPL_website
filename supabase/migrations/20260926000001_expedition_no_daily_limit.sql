-- No more daily launch limit.
--
-- launch_expedition capped a collector at one launch per Eastern day (two
-- for patrons). The ceiling that remains is the one that was always the
-- real shape of the feature: one run OUT per tier at a time ('tier already
-- out'), so a collector can have a scout, a raid and a legend hunt all in
-- the field, and relaunch a tier the moment its run is claimed. Everything
-- else in the function is exactly as 20260914000001 left it; the
-- thirteen-argument convoy launch (20260917000001) delegates here and
-- picks the change up unchanged.
