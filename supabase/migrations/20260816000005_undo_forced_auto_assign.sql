...
-- swap_roster_players (20260810000001) moved team_id without touching the new
-- auto_assigned_from_lot_id column: a swapped-out player would keep the stamp
-- of whatever lot forced their original auto-assignment. Now that Finding 1
-- makes undo reachable after the draft completes -- exactly when swaps happen
-- -- an undo of that old lot would refund the swapped-*to* team and yank the
-- swapped-in player back to the pool, stranding them. Clear the stamp on both
-- players so a swap severs them from any cascade that once assigned them.
-- Restated in full (same signature) with every existing rule preserved.
...
begin
  ...
  update public.players
  set team_id = v_right_team_id, acquisition = v_left_acquisition, auto_assigned_from_lot_id = null
  where id = v_left.id;
  update public.players
  set team_id = v_left_team_id, acquisition = v_right_acquisition, auto_assigned_from_lot_id = null
  where id = v_right.id;
end;
$$;
