-- Qualify betting_profiles.balance so PL/pgSQL does not confuse the column
-- with the output parameter named balance.

create or replace function public.vote_daily_banger(
  p_post_id text,
  p_voter_id uuid,
  p_discord_id text,
  p_vote text
...
begin
  ...
  perform 1
   ...
   where discord_id = p_discord_id
     for update;
  if not found then
    raise exception 'unknown betting user';
  end if;

  if not exists (
    select 1
      from public.daily_banger_checks
     where check_date = v_date
  ...
  if v_inserted then
    ...
    update public.betting_profiles as profile
       set balance = profile.balance + 200
     where profile.discord_id = p_discord_id
    returning profile.balance into v_balance;
  else
    select profile.balance
      into v_balance
      from public.betting_profiles as profile
     where profile.discord_id = p_discord_id;
  end if;
