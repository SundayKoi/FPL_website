...
export function rarityGuide(season: string | null, league: "premier" | "academy" = "premier"): RaritySection[] {
  ...
  const stamps: RarityEntry[] = [
    ...
    },
    {
      ...
      how: "A chance on every expedition that goes badly, or strangely. Read by Fantasy and dust.",
      odds: "earned",
  ...
  const wear: RarityEntry[] = [
    {
      ...
      how: "Every copy, from its own record: an expedition launch and a scored Fantasy week each count one. A copy that stays on the shelf stays Factory New.",
      odds: "every copy",
    ...
    },
    {
      ...
      how: "Your call, once, from the copy's drawer on your collection. A slabbed copy can never be fielded again — not on an expedition, not in Fantasy — and the seal can never be taken off. It can still be sold, traded or dusted.",
      odds: "your choice",
