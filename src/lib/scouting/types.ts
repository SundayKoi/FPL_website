...
export interface ScoutSource extends ScoutHistory { opponentName: string; /** Team whose roster the source roster represents. */ teamName?: string; /** Existing team crest, when the current card identity has one. */ teamImageUrl?: string | null; currentSeason: string; nextFixture: ScoutFixtureRow; roster: ScoutRosterPlayer[]; /** Riot-ingested games are authoritative for player champion pools when draft roles/names are absent. */ ingestedGames?: IngestedScoutingGame[]; inhousePlayerStats?: InhousePlayerStats[]; }
