...
describe("drafterAccess", () => {
  ...
  it("denies signed-out visitors", async () => {
    ...
    expect(await drafterAccess()).toEqual({ signedIn: false, allowed: false, inconclusive: false, staff: false });
  });
  ...
  it("allows premium-role holders in the premium guild", async () => {
    ...
    expect(await drafterAccess()).toEqual({ signedIn: true, allowed: true, inconclusive: false, staff: false });
  ...
  it("lets site admins through without the Discord check", async () => {
    ...
    expect(await drafterAccess()).toEqual({ signedIn: true, allowed: true, inconclusive: false, staff: true });
  ...
  it("fails open when Discord is inconclusive, closed for accounts without Discord", async () => {
    ...
    expect(await drafterAccess()).toEqual({ signedIn: true, allowed: true, inconclusive: true, staff: false });
