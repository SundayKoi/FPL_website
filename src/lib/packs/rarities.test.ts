...
describe("the finishes' CSS", () => {
  it("defines every layer PlayerCard3D names for a finish", async () => {
    ...
    for (const layer of ["card-shiny-art", "card-shiny-burst", "card-secret-frame", "card-stattrak-led", "card-wear-ww", "card-wear-bs", "card-slab", "card-ink-write", "card-ink-pen", "card-coin"]) {
      expect(css, layer).toContain(`@utility ${layer}`);
    }
    // The ink sweep runs only while the card is held.
    expect(css).toContain('[data-motion="live"] .card-ink-write');
    expect(css).toContain('[data-motion="live"] .card-ink-pen');
  });
});
