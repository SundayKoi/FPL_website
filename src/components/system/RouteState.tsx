...
export function RouteLoadingState() {
  return (
    <main className="bg-hash flex flex-1" aria-busy="true">
      <div className="mx-auto w-full max-w-7xl px-4 py-12 sm:px-6 sm:py-16">
        <p role="status" className="label-dash text-coral">
          Loading page…
        </p>
        <div aria-hidden="true" className="mt-5 animate-pulse space-y-6">
          <div className="h-12 w-64 max-w-full rounded bg-panel" />
          <div className="h-4 w-[34rem] max-w-full rounded bg-line/70" />
          <div className="grid gap-4 pt-5 sm:grid-cols-2 lg:grid-cols-3">
            {Array.from({ length: 6 }, (_, index) => (
              <div key={index} className="card-brand h-36 bg-panel/70" />
            ))}
          </div>
        </div>
      </div>
...
export function RouteErrorState({ onRetry }: { onRetry: () => void }) {
  return (
    <main className="bg-hash flex flex-1 items-center justify-center px-6 py-20 text-center">
      <section className="card-brand w-full max-w-xl p-8" role="alert">
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          <button type="button" onClick={onRetry} className="btn-coral px-4 py-2 text-xs">
            Try again
          </button>
