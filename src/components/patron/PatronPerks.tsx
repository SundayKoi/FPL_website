...
export default function PatronPerks({
  ...
  return (
    <div className={`rounded-xl border border-line bg-black/10 p-4 ${className}`}>
      ...
      <ul className={`mt-3 grid gap-2 text-sm leading-6 text-steel ${compact ? "sm:grid-cols-2" : "sm:grid-cols-2"}`}>
        {perks.map((perk) => (
          <li key={perk.key}>
      ...
      <p className="mt-3 text-xs leading-5 text-steel">{PATRON_FAIRNESS_NOTE}</p>
