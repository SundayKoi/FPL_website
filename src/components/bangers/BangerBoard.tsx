  ...
  return (
    <main className="min-h-screen overflow-hidden bg-jungle bg-hash">
      ...
      <section className="mx-auto flex max-w-6xl flex-col gap-12 px-5 pb-16 sm:px-10">
        ...
        <div className="order-2 relative overflow-hidden rounded-3xl border border-coral/40 bg-gradient-to-br from-[#173b2c] to-jungle-card p-6 shadow-[0_18px_50px_rgba(0,0,0,0.25)]"><div className="absolute -right-4 -top-8 text-8xl opacity-30" aria-hidden="true">🍌</div><p className="label-dash">From the archive</p><h2 className="mt-3 font-display text-3xl font-bold uppercase italic text-white">{settings.randomTitle}</h2><div className="mt-7 rounded-2xl border border-white/10 bg-jungle/60 p-4">{randomPost ? <><div className="mb-3 flex items-center gap-2 text-xs text-white/45"><span className="text-lg">🐒</span> @Stuart69Davis · {postDate(randomPost.publishedAt)}</div><p className="text-lg leading-snug text-white/85">{randomPost.text}</p><div className="mt-5"><VoteButtons post={withVotes(randomPost, votes)} currentVote={votes[randomPost.id]} onVote={(nextVote) => vote(randomPost.id, nextVote)} /></div><div className="mt-3 text-xs font-bold uppercase tracking-[0.15em] text-banana">{percent(withVotes(randomPost, votes))} banger rating</div></> : <p className="text-sm leading-relaxed text-white/45">The all-time verified archive is empty. Random pulls will unlock when real posts are connected.</p>}</div><button type="button" onClick={() => setRandomPost(pickRandomPost(posts))} disabled={posts.length === 0} className="mt-5 w-full rounded-full border border-coral/70 px-4 py-3 text-xs font-bold uppercase tracking-[0.18em] text-coral transition hover:bg-coral hover:text-jungle disabled:cursor-not-allowed disabled:opacity-40">{posts.length === 0 ? "Awaiting verified posts" : "Pull another banana ↻"}</button></div>
      </section>
    </main>
  );
}
