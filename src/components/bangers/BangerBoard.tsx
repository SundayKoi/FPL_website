...
function TweetCard({ post, currentVote, onVote, featured = false }: { post: BangerPost; currentVote?: Vote; onVote: (vote: Vote) => void; featured?: boolean }) {
  return (
    <article className={`group relative overflow-hidden rounded-2xl border bg-jungle-card/90 p-5 transition hover:-translate-y-0.5 hover:border-banana/60 ${featured ? "border-banana/50 shadow-[0_14px_50px_rgba(0,0,0,0.3)]" : "border-white/10"}`}>
      ...
      <div className="mb-5 pr-10"><TweetIdentity date={postDate(post.publishedAt)} /></div>
    ...
    <main className="min-h-screen overflow-hidden bg-jungle bg-hash">
      ...
      <section className="mx-auto max-w-6xl px-5 pt-12 sm:px-10">
        <div className="rounded-3xl border border-mint/30 bg-gradient-to-br from-[#173b2c] to-jungle-card p-6 shadow-[0_18px_50px_rgba(0,0,0,0.25)] sm:p-8">
          ...
          {dailyBanger ? <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end"><div><TweetIdentity date={postDate(dailyBanger.publishedAt)} /><p className="mt-4 max-w-3xl text-xl leading-snug text-white/85">{dailyBanger.text}</p></div><div><p className="mb-3 text-xs font-bold uppercase tracking-[0.16em] text-mint">Vote once today · get $100</p><VoteButtons post={dailyBanger} currentVote={dailyVote} onVote={voteDaily} /><p className="mt-3 text-xs text-white/45">{dailyMessage || `Refreshes ${new Intl.DateTimeFormat("en-US", { hour: "numeric", minute: "2-digit", timeZone: "UTC" }).format(new Date(dailyBanger.endsAt))} UTC`}</p></div></div> : <p className="mt-6 text-sm text-white/50">No verified tweets are available for today&apos;s check yet.</p>}
        </div>
      </section>
      ...
      <section className="mx-auto flex max-w-6xl flex-col gap-12 px-5 pb-16 sm:px-10">
        ...
        <div className="order-2 relative overflow-hidden rounded-3xl border border-coral/40 bg-gradient-to-br from-[#173b2c] to-jungle-card p-6 shadow-[0_18px_50px_rgba(0,0,0,0.25)]"><div className="absolute -right-4 -top-8 text-8xl opacity-30" aria-hidden="true">🍌</div><p className="label-dash">From the archive</p><h2 className="mt-3 font-display text-3xl font-bold uppercase italic text-white">{settings.randomTitle}</h2><div className="mt-7 rounded-2xl border border-white/10 bg-jungle/60 p-4">{randomPost ? <><TweetIdentity date={postDate(randomPost.publishedAt)} /><p className="mt-4 text-lg leading-snug text-white/85">{randomPost.text}</p><div className="mt-5"><VoteButtons post={withVotes(randomPost, votes)} currentVote={votes[randomPost.id]} onVote={(nextVote) => vote(randomPost.id, nextVote)} /></div><div className="mt-3 text-xs font-bold uppercase tracking-[0.15em] text-banana">{percent(withVotes(randomPost, votes))} banger rating</div></> : <p className="text-sm leading-relaxed text-white/45">The all-time verified archive is empty. Random pulls will unlock when real posts are connected.</p>}</div><button type="button" onClick={() => setRandomPost(pickRandomPost(posts))} disabled={posts.length === 0} className="mt-5 w-full rounded-full border border-coral/70 px-4 py-3 text-xs font-bold uppercase tracking-[0.18em] text-coral transition hover:bg-coral hover:text-jungle disabled:cursor-not-allowed disabled:opacity-40">{posts.length === 0 ? "Awaiting verified posts" : "Pull another banana ↻"}</button></div>
      </section>
    </main>
  );
}
