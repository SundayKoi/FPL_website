...
export default function BangerBoard() {
  ...
  const [randomPost, setRandomPost] = useState<BangerPost>(BANGER_POSTS[0]);
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-jungle bg-hash">
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
            ...
            <div className="rounded-2xl border border-banana/30 bg-jungle-card/80 p-6 shadow-[0_20px_60px_rgba(0,0,0,0.25)]">
              ...
              <div className="mt-3 flex items-end gap-3"><span className="font-display text-6xl font-bold italic text-banana">87%</span><span className="pb-2 text-sm text-white/45">community certified</span></div>
              <div className="mt-4 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full w-[87%] rounded-full bg-gradient-to-r from-coral via-banana to-mint" /></div>
              <p className="mt-3 text-sm text-white/45">A healthy canopy of strong opinions. Keep voting to move the canopy.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-5 py-12 sm:px-10">
        ...
        <div className="grid gap-4 lg:grid-cols-3">{rankedPosts.map((post, index) => <div key={post.id} className={`${index === 0 ? "lg:-translate-y-3" : ""} rounded-2xl border border-white/10 bg-jungle-card/70 p-4`}><div className="mb-3 flex items-center justify-between"><span className={`flex h-8 w-8 items-center justify-center rounded-full font-display text-lg font-bold ${index === 0 ? "bg-banana text-jungle" : "bg-white/10 text-white/60"}`}>{index + 1}</span><span className="text-xs font-bold uppercase tracking-[0.15em] text-banana">{percent(post)} banger</span></div><p className="min-h-16 text-sm leading-relaxed text-white/75">{post.text}</p><div className="mt-4 text-xs text-white/35">{post.bangerVotes + post.midVotes} community votes · {postDate(post.publishedAt)}</div></div>)}</div>
      </section>

      <section className="mx-auto grid max-w-6xl gap-8 px-5 pb-16 sm:px-10 lg:grid-cols-[0.72fr_1.28fr]">
        <div className="relative overflow-hidden rounded-3xl border border-coral/40 bg-gradient-to-br from-[#173b2c] to-jungle-card p-6 shadow-[0_18px_50px_rgba(0,0,0,0.25)]"><div className="absolute -right-4 -top-8 text-8xl opacity-30" aria-hidden="true">🍌</div><p className="label-dash">From the archive</p><h2 className="mt-3 font-display text-3xl font-bold uppercase italic text-white">Random<br /><span className="text-coral">pull</span></h2><div className="mt-7 rounded-2xl border border-white/10 bg-jungle/60 p-4"><div className="mb-3 flex items-center gap-2 text-xs text-white/45"><span className="text-lg">🐒</span> @Stuart69Davis · {postDate(randomPost.publishedAt)}</div><p className="text-lg leading-snug text-white/85">{randomPost.text}</p></div><button type="button" onClick={() => setRandomPost(pickRandomPost(BANGER_POSTS))} className="mt-5 w-full rounded-full border border-coral/70 px-4 py-3 text-xs font-bold uppercase tracking-[0.18em] text-coral transition hover:bg-coral hover:text-jungle">Pull another banana ↻</button></div>
        <div><div className="mb-6 flex items-end justify-between gap-4"><div><p className="label-dash">The current canopy</p><h2 className="mt-2 font-display text-3xl font-bold uppercase italic text-white sm:text-4xl">Last 45 days</h2></div><span className="text-xs uppercase tracking-[0.15em] text-white/35">{recentPosts.length} transmissions</span></div><div className="space-y-4">{recentPosts.map((post) => <TweetCard key={post.id} post={withVotes(post, votes)} currentVote={votes[post.id]} onVote={(nextVote) => vote(post.id, nextVote)} featured={post.id === recentPosts[0].id} />)}</div></div>
      </section>
    </main>
  );
}
