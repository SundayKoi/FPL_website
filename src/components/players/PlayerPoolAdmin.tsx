...
export default function PlayerPoolAdmin({
  ...
  return <section aria-label="Player pool administration" className="mb-8 rounded border border-danger/40 bg-danger/5 p-4 sm:p-6">
    ...
    <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_8rem_8rem_minmax(0,1.5fr)_auto]">
      <input aria-label="Player name" value={form.display_name} onChange={(e) => setForm({ ...form, display_name: e.target.value })} placeholder="Player name" className="rounded border border-border bg-canvas px-3 py-2 text-sm text-white" />
      <select aria-label="Player role" value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value as RoleSection["key"] })} className="rounded border border-border bg-canvas px-3 py-2 text-sm text-white">{roles.map((role) => <option key={role} value={role}>{role.toUpperCase()}</option>)}</select>
      <input aria-label="Player rank" value={form.rank} onChange={(e) => setForm({ ...form, rank: e.target.value })} placeholder="Rank" className="rounded border border-border bg-canvas px-3 py-2 text-sm text-white" />
      <input aria-label="Player OP.GG URL" value={form.opgg_url} onChange={(e) => setForm({ ...form, opgg_url: e.target.value })} placeholder="https://op.gg/..." className="rounded border border-border bg-canvas px-3 py-2 text-sm text-white" />
      <div className="flex gap-2"><button type="button" onClick={() => void save()} disabled={saving} className="rounded border border-primary px-3 py-2 text-sm font-semibold text-primary">{editingId ? "Save" : "Add"}</button>{editingId ? <button type="button" onClick={reset} className="rounded border border-border px-3 py-2 text-sm text-muted">Cancel</button> : null}</div>
    </div>
    <ul className="mt-5 divide-y divide-border/50">{players.map((player) => <li key={player.id} className="py-2 text-sm text-white"><div className="flex flex-wrap items-center justify-between gap-3"><span>{player.display_name} <span className="text-muted">({player.role}, {player.rank ?? "—"})</span></span><span className="flex gap-2"><button type="button" onClick={() => beginEdit(player)} className="text-primary underline">Edit</button><button type="button" onClick={() => void remove(player)} disabled={saving} className="text-red-400 underline">Remove</button></span></div>{identityLeague && identitySeason ? <PlayerIdentityAdmin playerPoolId={player.id} league={identityLeague} season={identitySeason} currentLink={identityLinks.find((link) => link.playerPoolId === player.id) ?? null} profiles={profilesForPlayer(player.id)} /> : null}</li>)}</ul>
  </section>;
}
