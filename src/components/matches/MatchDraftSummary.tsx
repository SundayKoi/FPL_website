...
export function ChampionIcon({ name, banned = false, size = "h-9 w-9" }: { name: string | null; banned?: boolean; size?: string }) {
  ...
  if (!src) {
    return <span className={`${size} rounded border border-dashed border-border-subtle/60 bg-canvas/40`} title={name ?? "Skipped"} />;
  }
  return (
    <span className={`relative inline-block ${size} overflow-hidden rounded border border-border-subtle/60`} title={name ?? undefined}>
