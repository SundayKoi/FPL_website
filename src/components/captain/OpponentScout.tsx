...
export default function OpponentScout({
  ...
  return <section aria-labelledby="scouting-heading" className="mt-8 space-y-4">
    ...
    {mode === "inhouse" || hasDrafts ? <><ScoutPlayerPools data={data} scope={scope} unavailable={source.roster.length === 0} mode={mode} onModeChange={() => setMode((current) => current === "regular" ? "inhouse" : "regular")} inhousePlayers={source.inhousePlayerStats ?? []} />{mode === "regular" ? <><ScoutPatterns data={data} /><ScoutPastDrafts drafts={data.pastDrafts} /></> : null}</> : <p className="card-brand p-5 text-sm text-steel">{emptyDraftCopy}</p>}
  </section>;
}
