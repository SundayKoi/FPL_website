    ...
    <main className="page-backdrop flex-1">
      <div className="mx-auto w-full max-w-[1800px] px-4 py-12 sm:px-6 sm:py-16">
        <section aria-labelledby="preseason-home-title" className="card-brand overflow-hidden p-5 sm:p-8 xl:p-10">
          <div className="grid gap-8 lg:grid-cols-[1.3fr_0.7fr] lg:items-end">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
              <div className="rounded-lg border border-prestige/50 bg-prestige/10 p-5">
                ...
                <p className="mt-2 text-sm leading-6 text-muted">The Season 5 draft goes live.</p>
              </div>
                <div className="rounded-lg border border-league-secondary/40 bg-league-secondary/10 p-5">
                <span className="label-dash text-league-secondary">FIRST GAME</span>
                <p className="mt-3 text-xl font-semibold text-white">Monday, August 17</p>
                <p className="mt-2 text-sm leading-6 text-muted">The regular season begins.</p>
              </div>
            </div>
          </div>
        </section>
                ...
                <article key={team.id} className="card-brand overflow-hidden">
                  ...
                  <div className="p-5">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <span className="font-mono text-xs font-semibold text-muted">#{team.nominationPosition} · {team.abbreviation}</span>
                        <h3 className="mt-2 text-xl font-semibold text-white">{team.name}</h3>
                        <p className="mt-1 text-xs uppercase tracking-[0.12em] text-muted">Captain {team.captainName}</p>
                      </div>
                      <span className="rounded-full border border-league-secondary/40 bg-league-secondary/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-league-secondary">{team.rosterCount}/5</span>
                    </div>
                    <div className="mt-5 flex items-end justify-between gap-3">
                      <div>
                        <span className="label-dash">POINTS LEFT</span>
                        <p className="mt-1 font-mono text-3xl font-bold text-prestige">{team.pointsRemaining} pts left</p>
                      </div>
                    ...
                    <div className="mt-5 border-t border-border-subtle/60 pt-4">
                      ...
                      {team.draftedPlayers.length > 0 ? (
                          ...
                          {team.draftedPlayers.map((player) => (
                            <li key={player.id} className="flex items-center justify-between gap-3 text-sm">
                              <span className="min-w-0 truncate font-semibold text-white">
                                <span className="mr-2 font-mono text-[10px] text-muted">{player.role.toUpperCase()}</span>
                                {player.displayName}
                              </span>
                        ...
                        <p className="mt-2 text-xs text-muted">No players drafted yet.</p>
                      )}
                    </div>
                  </div>
                </article>
        ...
        <section aria-label="Preseason notes" className="mt-6 grid gap-4 md:grid-cols-3 xl:mt-8">
          ...
          <div className="card-brand p-5"><span className="label-dash">02 · PLAYER POOL</span><p className="mt-3 text-sm leading-6 text-muted">Remaining players are clearly visible and sorted by rank. Captains stay pinned at the top of each role.</p></div>
          <div className="card-brand p-5"><span className="label-dash">03 · OPENING WEEK</span><p className="mt-3 text-sm leading-6 text-muted">The first regular-season games begin Monday, August 17. The full league dashboard takes over then.</p></div>
        </section>
      </div>
    </main>
  );
