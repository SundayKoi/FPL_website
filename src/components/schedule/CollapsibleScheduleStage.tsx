    ...
    <div id={stageId} className="card-brand scroll-mt-24 overflow-hidden">
      <div className="border-b border-line">
        <button
          ...
          className="flex w-full flex-wrap items-baseline justify-between gap-2 px-4 py-3 text-left hover:bg-panel/40 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-coral"
        >
          <span className="type-display text-xl">{label}</span>
          <span className="flex items-center gap-3 text-xs text-steel">
            <span>{note}</span>
            <span aria-hidden className="text-base leading-none text-coral">
              {isOpen ? "−" : "+"}
            </span>
          </span>
        </button>
