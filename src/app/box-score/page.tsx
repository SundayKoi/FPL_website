...
export const metadata: Metadata = {
  title: "Box Score — FPL",
  description: "Identify today's Premier player from a completed game box score.",
};

export default async function BoxScorePage() {
  let game;
  try {
    game = await getBoxScoreGame("premier");
  } catch (error) {
    if (error instanceof BoxScoreError && error.code === "FORBIDDEN") redirect("/premium");
    return <BoxScoreUnavailable league="Premier" />;
  }
