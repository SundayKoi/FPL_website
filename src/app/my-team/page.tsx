...
function fixtureBelongsToLeague(
  fixture: Pick<FixtureRow, "team_a" | "team_b">,
  teams: LeagueTeam[],
  league: LeagueKey,
): boolean {
...
function idPairBelongsToLeague(
  teamAId: string,
  teamBId: string,
  teamIds: Set<string>,
  league: LeagueKey,
): boolean {
  const teamA = teamIds.has(teamAId);
  const teamB = teamIds.has(teamBId);
  return league === "academy" ? teamA || teamB : teamA && teamB;
}
  ...
  if (dashboard.isAdmin) {
    try {
      ...
      const fixtures = ((fixturesResult.data as FixtureRow[] | null) ?? [])
        .filter((fixture) => fixtureBelongsToLeague(fixture, dashboard.teams, league));
      const reports = ((reportsResult.data as MatchReport[] | null) ?? [])
        .filter((report) => idPairBelongsToLeague(report.team_a_id, report.team_b_id, teamIds, league));
      const games = (gamesResult.data as MatchReportGame[] | null) ?? [];
      const codes = ((codesResult.data as MatchCode[] | null) ?? [])
        .filter((code) => idPairBelongsToLeague(code.team_a_id, code.team_b_id, teamIds, league));
      const memberships = ((membershipsResult.data as RosterMembershipRow[] | null) ?? [])
        .filter((membership) => teamIds.has(membership.league_team_id));
...
export default async function MyTeamPage({ searchParams }: { searchParams: SearchParams }) {
  return MyTeamPageView({ league: "premier", searchParams });
}
