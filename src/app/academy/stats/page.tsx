...
export default async function AcademyStatsPage({ searchParams }: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  return <main className="grid-neon flex-1"><div className="mx-auto w-full max-w-[1800px] px-4 py-12 sm:px-6 sm:py-16"><header className="border-b border-line pb-8"><span className="mono-label"><span className="text-cyan">&gt;</span> Academy League Data</span><h1 className="type-display text-neon mt-3 text-5xl sm:text-6xl">Academy Stats</h1><p className="mt-4 max-w-2xl text-lg leading-8 text-steel">Stats views for the Academy league, its first season.</p></header><div className="mt-10"><StatsTabs initialPlayer={first(params.player)} initialTab={first(params.tab)} initialSeason={first(params.season)} initialPhase={first(params.phase)} teamNames={teamNames} allowedSeasons={[seasons.academy]} /></div></div></main>;
}
