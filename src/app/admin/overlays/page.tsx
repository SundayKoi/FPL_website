...
const GROUPS: OverlayMockup["group"][] = ["kept", "sports", "tcg", "digital"];
    ...
    <main className="bg-hash mx-auto flex w-full max-w-[1400px] flex-1 flex-col gap-12 px-6 py-16">
      <header className="flex flex-col gap-3">
        ...
        <p className="max-w-3xl text-sm text-steel">
          {OVERLAY_MOCKUPS.length} proposed treatments beyond the foil ladder, on real cards from{" "}
                ...
                <div className="flex flex-wrap gap-6">
                  {entry.variants
                    ? entry.variants.map((variant) => (
                        <div key={`${entry.key}-${variant.label}`} className="flex flex-col items-center gap-2">
                          <PlayerCard3D
                            card={featured[0]}
                            interactive
                            forceFoil={entry.foil}
                            foilType={entry.foil ? "prisma" : null}
                            overlay={{ ...entry, front: variant.front, chip: variant.chip, artEcho: variant.artEcho }}
                          />
                          <span className="text-xs text-steel">{variant.label}</span>
                        </div>
                      ))
